...
class SignalService {
  ...
  Future<void> ensureBootstrapped(Uint8List identitySecretKeyBytes) {
    return _withLock('bootstrap:$_myUserId', () async {
      ...
      if (freshLocalStore) {
        ...
        // Those published prekeys' private halves are gone, so every session a
        // peer builds from one fails forever with "No such prekey", and the
        // count-based top-up below would skip regenerating because the server
        // still looks stocked. Wipe the stale bundle so we republish a fresh,
        // fully-matching set — this is the root-cause fix for the persistent
        // cross-device "Не удалось расшифровать".
  ...
  /// Repairs the "No such prekey" failure mode without any local reset: this
  /// account's *published* one-time prekeys were generated by a previous local
  /// store whose private halves are gone, so incoming sessions keep failing
  /// even though the current local store is otherwise intact. Wipes the stale
  /// server bundle and republishes a fresh signed prekey + full one-time set
  /// whose private halves live in the *current* store. Cheap enough to call
  /// once the first time a decrypt fails with an InvalidKeyIdException (see
  /// `ChatRepository._prewarmSignalDecryption`). Because both parties in a
  /// broken conversation are typically in the same re-handshake loop, once
  /// each republishes, the next handshake claims a live prekey and succeeds —
  /// the conversation self-heals with no manual "reset key".
  Future<void> republishOwnPreKeys() {
    return _withLock('bootstrap:$_myUserId', () async {
      ...
      final startId = await _store.reserveNextPreKeyIds(_oneTimePreKeyTarget);
      final records = generatePreKeys(startId, _oneTimePreKeyTarget);
