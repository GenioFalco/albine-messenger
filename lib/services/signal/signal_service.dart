...
class SignalService {
  ...
  Future<void> ensureBootstrapped(Uint8List identitySecretKeyBytes) {
    return _withLock('bootstrap:$_myUserId', () async {
      final freshLocalStore = !await _store.hasIdentity();
      if (freshLocalStore) {
        ...
        // The local prekey store is empty, but the server may still hold this
        // account's *previous* prekey bundle (cleared browser data, a
        // different origin such as localhost vs 127.0.0.1, or a key reset)
        // whose private halves are gone. Wipe it so the signed prekey below is
        // republished clean.
        await _directory.deleteAllOwnPreKeys(_myUserId).catchError((_) {});
      }

      final identityKeyPair = await _store.getIdentityKeyPair();
      await _rotateSignedPreKeyIfNeeded(identityKeyPair);
      // Deliberately NO one-time prekeys — see [republishOwnPreKeys].
    });
  }

  /// Republishes a clean bundle after a decrypt failure. Deletes whatever this
  /// account has on the server and reinstates a fresh signed prekey whose
  /// private half lives in the *current* local store.
  ///
  /// **One-time prekeys are deliberately not used at all** (not here, not in
  /// [ensureBootstrapped]). They were the entire source of the recurring
  /// "InvalidKeyIdException - No such prekey: N": a pool that accumulates
  /// server-side and desyncs from the local store on any store loss (cleared
  /// data / different origin / key reset), which the count-based top-up then
  /// refuses to heal. X3DH establishes a perfectly good session from the
  /// signed prekey alone — and the signed prekey is a single row (PK =
  /// user_id) that every bootstrap overwrites, so it can never orphan the way
  /// the one-time pool did. The trade-off is the marginal extra
  /// forward-secrecy a one-time prekey gives the *very first* message of a new
  /// session (a well-understood, optional X3DH ingredient); reliability wins
  /// here after this bug class broke decryption repeatedly. The Double Ratchet
  /// still provides full forward secrecy for every message once a session is
  /// running — this only affects that first-message edge.
  Future<void> republishOwnPreKeys() {
    return _withLock('bootstrap:$_myUserId', () async {
      if (!await _store.hasIdentity()) return;
      final identityKeyPair = await _store.getIdentityKeyPair();

      await _directory.deleteAllOwnPreKeys(_myUserId);

      final nextSignedId = (await _store.currentSignedPreKeyId() ?? 0) + 1;
      final signed = generateSignedPreKey(identityKeyPair, nextSignedId);
      await _store.storeSignedPreKey(nextSignedId, signed);
      await _store.setCurrentSignedPreKeyId(nextSignedId);
      await _directory.upsertSignedPreKey(
        userId: _myUserId,
        keyId: nextSignedId,
        publicKey: signed.getKeyPair().publicKey.serialize(),
